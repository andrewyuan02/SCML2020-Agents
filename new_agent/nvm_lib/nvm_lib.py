...
class NVMLib:
    def __init__(self,
        ...
        self.mpnvp_feasible_sols = NVMLib.read_qtty_feasible_domain(self.mpnvp_number_of_periods, self.mpnvp_quantities_domain_size)

        # Check if the data exists.
        if self.check_if_data_exists():
            # Get the data for quantities
            q_uncertainty_model = NVMLib.get_json_dict(f'/Users/jtsatsaros/SCML/agent/nvm_lib/data/dict_qtty_num_intermediate_products_{self.num_intermediate_products}.json')
            ...
            self.expectations_q_min_inn = {t: NVMLib.compute_min_expectation(self.q_inn_uncertainty_model[str(t + 1)], self.mpnvp_quantities_domain_size)
                                           for t in range(game_length - 1)}
            self.expectations_q_min_out = {t: NVMLib.compute_min_expectation(self.q_out_uncertainty_model[str(t + 1)], self.mpnvp_quantities_domain_size)
                                           for t in range(game_length - 1)}
            ...
            prices = NVMLib.get_json_dict(f'/Users/jtsatsaros/SCML/agent/nvm_lib/data/dict_price_num_intermediate_products_{self.num_intermediate_products}.json')
    ...
    def check_if_data_exists(self) -> bool:
        ...
        :return:
        """
        if not os.path.isfile(f'/Users/jtsatsaros/SCML/agent/nvm_lib/data/dict_qtty_num_intermediate_products_{self.num_intermediate_products}.json'):
            raise Exception(f'The uncertainty model for quantities could not be found at data/dict_qtty_num_intermediate_products_{self.num_intermediate_products}.json')
        if not os.path.isfile(f'/Users/jtsatsaros/SCML/agent/nvm_lib/data/dict_price_num_intermediate_products_{self.num_intermediate_products}.json'):
            raise Exception(f'The uncertainty model for prices could not be found at data/dict_price_num_intermediate_products_{self.num_intermediate_products}.json')
        return True

    def get_complete_plan(self, current_time: int, verbose: bool = False) -> Optional[List[Tuple[int,int,int]]]:
        """
        ...
        slice_expectations_q_min_out = {t - current_time: self.expectations_q_min_out[t] for t in range(current_time, end)}
        slice_expectations_q_min_inn = {t - current_time: self.expectations_q_min_inn[t] for t in range(current_time, end)}
        ...
        slice_prices_inn = {t - current_time: self.prices_inn[str(t)] if str(t) in self.prices_inn else 0.0 for t in range(current_time, end)}
        slice_prices_out = {t - current_time: self.prices_out[str(t)] if str(t) in self.prices_out else 0.0 for t in range(current_time, end)}
